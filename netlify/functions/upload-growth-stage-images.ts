  ...
  try {
    ...
    if (process.env.FIREBASE_SERVICE_ACCOUNT_JSON) {
      try {
        ...
        console.log('[upload-growth-stage-images] Loaded service account from environment variable');
      } catch (parseError) {
        console.error('[upload-growth-stage-images] Error parsing FIREBASE_SERVICE_ACCOUNT_JSON:', parseError);
      }
    }
    ...
    if (!serviceAccount) {
      try {
        ...
        console.log('[upload-growth-stage-images] Loaded service account from local file');
      } catch (fileError) {
        console.error('[upload-growth-stage-images] Error loading local service account:', fileError);
        throw fileError;
      }
    }

  ...
  try {
    ...
    if (process.env.FIREBASE_SERVICE_ACCOUNT_JSON) {
      try {
        ...
        console.log('[chat-upload-image] Loaded service account from environment variable');
      } catch (parseError) {
        console.error('[chat-upload-image] Error parsing FIREBASE_SERVICE_ACCOUNT_JSON:', parseError);
      }
    }
    ...
    if (!serviceAccount) {
      try {
        ...
        console.log('[chat-upload-image] Loaded service account from local file');
      } catch (fileError) {
        console.error('[chat-upload-image] Error loading local service account:', fileError);
        throw fileError;
      }
    }
    ...
    console.error('[chat-upload-image] Error initializing Firebase Admin:', error);
    throw error;
  }
} else {
  bucket = getStorage().bucket();
...
const handler: Handler = async (event: HandlerEvent): Promise<HandlerResponse> => {
  ...
  try {
    ...
    // Get the conversation ID (use default if not provided)
    const conversationId = fields.conversationId || 'default';
    ...
    const fullPath = `chat-images/${userId}/${conversationId}/${filename}`;
    ...
    console.error('[chat-upload-image] Error uploading image:', error);
